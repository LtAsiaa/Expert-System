...
public class Main extends Application {
    ...
    static ResourceBundle movieResources;
    ...
    public static void run() {
        controller.MainLabel.setStyle("-fx-font-size: 12px");
        Runnable runThread = () -> {
            clips.run();
            Platform.runLater(() -> {
                try {
    ...
    static void clicked(String who) throws Exception {
        ...
        try {
            ...
            String content = movieResources.getString(fv.getFactSlot("display").symbolValue());
    ...
    static void initClips() {
        ...
        try (FileInputStream fis = new FileInputStream("movie_en.properties")){
            try {
                movieResources = new PropertyResourceBundle((fis));
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    static private void nextUIState() throws Exception {
        ...
        for (int i = 0; i < pv.size(); i++) {
            Button b = new Button(movieResources.getString(pv.get(i).toString()));
            buttons.add(b);
            int finalI = i;
        ...
        controller.MainLabel.setText(movieResources.getString(fv.getFactSlot("display").symbolValue()));
    ...
    public void start(Stage primaryStage) throws Exception {
        ...
        primaryStage.setTitle("Family Movies");
